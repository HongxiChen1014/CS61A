...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    i, sum_dice, if_dice_one = 0, 0, False
    while i < num_rolls:
        num_dice = dice()
        sum_dice += num_dice
        if num_dice == 1:
    ...
    def helper(score0, score1):
        if who == 0:
            if running_high < score0 - last_score:
                ...
                print(new_high, "point(s)! That's a record gain for Player " + str(who) + "!")
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_score, max_count = -1, 0
    count = 1
    while count < 11:
        score = make_averaged(roll_dice, trials_count)(count, dice)
        if max_score < score:
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if num == num_rolls:
        score_hog = hog_pile(score + picky_piggy(opponent_score), opponent_score)
        if score_hog >= cutoff:
            return 0
        else:
            return num_rolls
