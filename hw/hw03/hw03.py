...
def preorder(t):
    ...
    if is_leaf(t):
        ...
        bs = sum([preorder(b) for b in branches(t)], [])
        return [label(t)] + bs
...
def has_path(t, word):
    ...
    # print("label(t)=",label(t),"len=",len(word),'w=',word[0])
...
def quadratic(x, a, b, c):
    ...
    if lower_bound(x) <= extreme_point <= upper_bound(x):
        return interval(min(lower_f, upper_f, extreme_f), max(lower_f, upper_f, extreme_f))
    else:
