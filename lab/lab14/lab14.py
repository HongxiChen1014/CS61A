...
def num_splits(s, d):
    ...
    exist = []
    def splits(a, b, s, d):
        if s == [] and abs(sum(a)-sum(b)) <= d and (b + a) not in exist:
            exist.append(a + b)
            return 1
        elif s == []:
            return 0
        else:
            splits_left = splits(a + [s[0]], b, s[1:], d)
            splits_right = splits(a, b+[s[0]], s[1:], d)
...
def reverse(lst):
    ...
    True
    """
    return foldl(lst, lambda a, z: Link(z, a), Link.empty)
